function createSatelliteControllers(deps) {
  ...
  const SAT_CACHE_FILE = path.join(SAT_CACHE_DIR, 'tle-cache.json');
  ...
  try {
    fs.mkdirSync(SAT_CACHE_DIR, { recursive: true });
  } catch (_) { /* cache dir best effort */ }
  ...
  const satellites = async (_req, res) => {
  ...
  const satellitesDebug = async (_req, res) => {
